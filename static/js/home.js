...
(function ($) {
  jQuery(document).ready(function () {
    ...
    if (projectCardHolder != null && projectCardHolder.children.length != 0) {
      showCards = $(".filtr-projects").filterizr({ layout: 'sameWidth' });
    }
    ...
    var showCardsHolder = document.getElementById("show-cards");
    if (showCardsHolder != null && showCardsHolder.children.length != 0) {
      projectCards = $(".filtr-projects").filterizr({ layout: 'sameWidth' });
    }

    function adjustShowCardsHeight() {
      if (!isMobile) { // no need to adjust height for mobile devices
        let showCards = document.getElementById("show-cards")
        if (showCards != null) {
          let el = showCards.children;
          let maxHeight = 0;
